...
JSValueRef get_property(JSContextRef ctx, JSObjectRef object, const char *name) {
...
void set_property(JSContextRef ctx, JSObjectRef object, const char *name, JSValueRef value) {
    JSStringRef key = JSStringCreateWithUTF8CString(name);
...
JSObjectRef parse_request_headers(JSContextRef ctx, const char *raw) {
    ...
    while (*cursor) {
        ...
        if (colon) {
            ...
            char *value = copy_trimmed_span(colon + 1, line_len - (size_t)(colon - cursor) - 1);
...
JSObjectRef make_request_object(JSContextRef ctx, ParsedRequest *request, JSObjectRef headers) {
...
JSObjectRef response_constructor(JSContextRef ctx, JSObjectRef constructor, size_t argumentCount,
    ...
    JSValueRef prototype = JSObjectGetProperty(ctx, constructor, prototype_name, NULL);
...
char *serialize_status_response(int status, const char *body) {
    ...
    int size = snprintf(NULL, 0, "HTTP/1.1 %d %s\r\nContent-Length: %zu\r\n\r\n%s", status, reason, body_len, body);
    if (size < 0) return NULL;
    char *serialized = malloc((size_t)size + 1);
    if (serialized == NULL) return NULL;

    snprintf(serialized, (size_t)size + 1, "HTTP/1.1 %d %s\r\nContent-Length: %zu\r\n\r\n%s", status, reason, body_len,
             body);
...
char *write_response_headers(JSContextRef ctx, JSObjectRef headers, char *cursor) {
    JSPropertyNameArrayRef names = JSObjectCopyPropertyNames(ctx, headers);
    size_t count = JSPropertyNameArrayGetCount(names);
...
char *serialize_response(JSContextRef ctx, JSObjectRef response) {
    ...
    int status_line_size = snprintf(NULL, 0, "HTTP/1.1 %d %s\r\n", status, reason);
    int body_size = snprintf(NULL, 0, "Content-Length: %zu\r\n\r\n%s", body_len, body);
    if (status_line_size < 0 || body_size < 0) {
        free(body);
        return NULL;
    }

    size_t total_size = (size_t)status_line_size + headers_size + (size_t)body_size;
...
void install_classes(Runtime *rt) {
    ...
    for (size_t i = 0; i < num_classes; i++) {
        ...
        *classes[i].class_ref = JSClassCreate(&def);
        ...
        JSObjectRef constructor = JSObjectMakeConstructor(rt->ctx, *classes[i].class_ref, classes[i].constructor);
        JSObjectSetProperty(rt->ctx, global, js_name, constructor, kJSPropertyAttributeNone, NULL);
...
JSValueRef console_log_callback(JSContextRef ctx, JSObjectRef func, JSObjectRef thisObject, size_t argumentCount,
                                const JSValueRef arguments[], JSValueRef *exception) {
    (void)func;
    (void)thisObject;
    (void)exception;
...
JSValueRef set_timeout_callback(JSContextRef ctx, JSObjectRef func, JSObjectRef thisObject, size_t argumentCount,
                                const JSValueRef arguments[], JSValueRef *exception) {
    (void)func;
    (void)thisObject;
    (void)exception;
...
JSValueRef kobun_serve_callback(JSContextRef ctx, JSObjectRef func, JSObjectRef thisObject, size_t argumentCount,
                                const JSValueRef arguments[], JSValueRef *exception) {
    (void)func;
    (void)thisObject;
    (void)exception;
...
JSObjectRef get_builtin_parent(Runtime *rt, const char *object_name) {
    ...
    JSValueRef value = JSObjectGetProperty(rt->ctx, global, js_object_name, NULL);
    ...
    JSObjectSetProperty(rt->ctx, global, js_object_name, object, kJSPropertyAttributeNone, NULL);
    JSStringRelease(js_object_name);
    return object;
}

void install_builtin(Runtime *rt, const char *object_name, const char *function_name,
    ...
    JSObjectRef func = JSObjectMakeFunctionWithCallback(rt->ctx, js_name, callback);
    JSObjectSetProperty(rt->ctx, parent, js_name, func, kJSPropertyAttributeNone, NULL);
    JSStringRelease(js_name);
}
